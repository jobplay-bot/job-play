...
export default function CareersDetailQualification({
  ...
  jobType.length === 0 && !category &&!updatedAt && !workingLocation && !website) return null;
    ...
    <div className="p-4 border rounded-lg bg-white shadow-sm mt-4 text-sm text-gray-700 space-y-4">
      ...
      {(jobType.length > 0 || category) && (
        <div className="mt-0 grid gap-1">
          ...
          <p className="font-semibold">{[...jobType, category].filter(Boolean).join(" / ")}</p>
        </div>
      )}

      {updatedAt &&<hr className="border-gray-200" />}
      {updatedAt && (
        <div className="mt-0 grid gap-1">
      ...
      {workingLocation &&<hr className=" border-gray-200" />}
      {workingLocation && (
        <div className="mt-0 grid gap-1">
      ...
      {foreignLanguages &&<hr className="border-gray-200" />}
      {foreignLanguages.length > 0 && (
        <div className="mt-0 grid gap-1">
          <h4 className="text-gray-400">언어 능력</h4>
          <ul className="list-disc ml-5">
            {foreignLanguages.map((lang, i) => (
              <li key={i}>{lang.language} ({lang.proficiency}) - {lang.description}</li>
            ))}
          </ul>
        </div>
      )}

      {website &&<hr className="pt-4 border-gray-200" />}
      {website && (
        <div className="m-0">
